<p><?php _e('Thank you for installing our plugin.', VG_Required_Taxonomies::$textname); ?></p>
...
ob_start();
?>
<p><?php _e('Available post types', VG_Required_Taxonomies::$textname); ?></p>
...
foreach ($post_types as $post_type) {
	...
	?>
	<div class="post-type-field post-type-<?php echo $key; ?>"><input type="checkbox" name="post_types[]" value="<?php echo $key; ?>" id="<?php echo $key; ?>" <?php checked(in_array($key, $enabled_post_types)); ?>> <label for="<?php echo $key; ?>"><?php echo $post_type_name; ?> </label></div>
<?php } ?>
<?php
...
ob_start();
?>
<p><?php _e('Force users to select at least one term of these taxonomies when creating posts:', VG_Required_Taxonomies::$textname); ?></p>
...
foreach ($post_types as $post_type) {
	...
	<div class="post-type-taxonomies post-type-taxonomies-<?php echo $key; ?>">
		...
		<?php if (empty($taxonomies)) { ?>
			<p><?php printf(__('%s don´t have taxonomies.', VG_Required_Taxonomies::$textname), $post_type_name); ?></p>
		<?php } ?>
		<?php
		foreach ($taxonomies as $taxonomy_key => $taxonomy) {
			...
			?>
			<div class="taxonomy-field taxonomy-<?php echo $key; ?> taxonomy-<?php echo $taxonomy_key; ?>"><input type="checkbox" name="taxonomies[<?php echo $key; ?>][]" value="<?php echo $taxonomy_key; ?>" id="<?php echo $taxonomy_key; ?>" <?php echo $disabled; ?> <?php checked(isset($enabled_taxonomies[$key]) && in_array($taxonomy_key, $enabled_taxonomies[$key])); ?>> <label for="<?php echo $taxonomy_key; ?>"><?php echo $taxonomy->label; ?> <?php echo $maybe_go_premium; ?></label></div>
		<?php } ?>
...
ob_start();
?>
<label><?php _e('Error message displayed when a required taxonomy is not selected. <br/><small>Use {taxonomy_name} for the taxonomy name.</small>', VG_Required_Taxonomies::$textname); ?> <br/>
	<input type="text" name="error_message" value="<?php echo $error_message; ?>" style="width: 100%; display: block;"/></label>
...
?>
<br/>
<button class="save-settings button button-primary" data-loading-text="<?php _e('Loading...', VG_Required_Taxonomies::$textname); ?>"  data-success-text="<?php _e('Settings saved', VG_Required_Taxonomies::$textname); ?>"><?php _e('Save settings', VG_Required_Taxonomies::$textname); ?></button>
